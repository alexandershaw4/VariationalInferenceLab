...
for iter_i = 1:maxIter
    ...
    RW     = Wsqrt * R;
    RWg    = sqrtGamma * RW;
    % perhaps we shuold use RWg not just R below so that the likelihood is
    % also k-th derivative informed
    ...
    if numel(all_elbo) > 1 && logL < all_elbo(end-1)
        ...
        while ~improved && tries < 8
            ...
            [logL_try] = quick_elbo_gc(y, f, m_try, m0, T, E, Wsqrt, sqrtGamma, H_prior, q);
            if logL_try > logL
...
function [logL] = quick_elbo_gc(y, f, m, m0, T, E, Wsqrt, sqrtGamma, H_prior, q)
...
logL_prior = -0.5 * ((m - m0)') * H_prior * (m - m0); % zero; structurally here for completeness
